...
import { useAuthContext } from "../../auth/context/useAuthContext"; // ✅ import auth context
...
const ReviewSubmit = () => {
  ...
  const { token, isAuthenticated } = useAuthContext(); // ✅ get token from context
  ...
  const handlePublish = async () => {
    ...
    // ✅ Check authentication via context first, then fallback to localStorage
    const authToken =
      token ||
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/properties", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${authToken}`, // ✅ use resolved token
        },
        body: formData,
      });
      ...
      navigate("/property-submitted"); // ✅ navigate to success page
    } catch (err: unknown) {
    ...
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.form}>
          {/* A. Basic Information */}
          <Section title="A. Basic Information" onEdit={() => navigate("/post-property")}>
          ...
          {/* B. Property Details */}
          <Section title="B. Property Details" onEdit={() => navigate("/post-property")}>
            <div className={styles.grid2}>
              <Row label="Area Size" value={data.areaSize ? `${data.areaSize} ${data.areaUnit || ""}` : undefined} />
              {isFieldVisible("yearBuilt") && <Row label="Year Built" value={data.yearBuilt} />}
              {isFieldVisible("bedrooms") && <Row label="Bedrooms" value={data.bedrooms} />}
              {isFieldVisible("furnishing") && <Row label="Furnishing" value={data.furnishing} />}
              {isFieldVisible("bathrooms") && <Row label="Bathrooms" value={data.bathrooms} />}
              {isFieldVisible("floors") && <Row label="Floors" value={data.floors} />}
              {isFieldVisible("parking") && <Row label="Parking" value={data.parking} />}
              {isFieldVisible("possession") && <Row label="Possession" value={data.possession} />}
              {isFieldVisible("facingDirection") && <Row label="Facing Direction" value={data.facing} />}
            </div>
          </Section>

          {/* C. Pricing Details */}
          <Section title="C. Pricing Details" onEdit={() => navigate("/post-property")}>
            <div className={styles.grid2}>
              <Row label="Price (PKR)" value={data.price ? `PKR ${formatPrice(data.price)}` : undefined} />
              <Row label="Price Negotiable" value={data.negotiable ? "Yes" : "No"} />
              {data.installmentAvailable && (
                <>
                  <Row label="Down Payment" value={data.downPayment ? `PKR ${formatPrice(data.downPayment)}` : undefined} />
                  <Row label="Monthly Installment" value={data.monthlyInstallment ? `PKR ${formatPrice(data.monthlyInstallment)}` : undefined} />
                  <Row label="Duration" value={data.duration} />
                </>
              )}
              <Row label="Installment Available" value={data.installmentAvailable ? "Yes" : "No"} />
              {data.monthlyRent && (
                <>
                  <Row label="Monthly Rent" value={`PKR ${formatPrice(data.monthlyRent)}`} />
                  <Row label="Security Deposit" value={data.securityDeposit ? `PKR ${formatPrice(data.securityDeposit)}` : undefined} />
          ...
          {/* D. Amenities */}
          <Section title="D. Amenities & Features" onEdit={() => navigate("/post-property")}>
          ...
          {/* E. Media */}
          <Section title="E. Media" onEdit={() => navigate("/media-and-details")}>
            ...
            <div className={styles.imgGrid}>
              {(data.imageFiles || []).slice(0, 8).map((img, i) => (
                <img key={i} src={img.url} alt={`prop-${i}`} className={styles.thumb} />
              ))}
            </div>
            {data.videoUrl && (
          ...
          {/* F. Location */}
          <Section title="F. Location" onEdit={() => navigate("/media-and-details")}>
            <div className={styles.grid2}>
          ...
          <div className={styles.terms}>
            <label>
              <input type="checkbox" checked={agree1} onChange={() => setAgree1(!agree1)} />
              I confirm all information is accurate and I am authorized to list this property *
            </label>
            <label>
              <input type="checkbox" checked={agree2} onChange={() => setAgree2(!agree2)} />
          ...
          <div className={styles.actions}>
            <button className={styles.draftBtn} onClick={handleBack} disabled={loading}>
              <Save size={16} /> Back
            </button>
